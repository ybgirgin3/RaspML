...
def load_dataset(dataset_folder, patch_side, border_width,identity, num): 
    ...
    """
    x_train = np.array(shape=(0, B, ps, ps), dtype='float32')
    y_train = np.array(shape=(0, 1, ps-2*r, ps-2*r), dtype='float32')
    x_val = np.array(shape=(0, B, ps, ps), dtype='float32')
    y_val = np.array(shape=(0, 1, ps-2*r, ps-2*r), dtype='float32')
    """

    x_train = np.array((0, B, ps, ps), dtype='float32')
    y_train = np.array((0, 1, ps-2*r, ps-2*r), dtype='float32')
    x_val   = np.array((0, B, ps, ps), dtype='float32')
    y_val   = np.array((0, 1, ps-2*r, ps-2*r), dtype='float32')
    ...
    for file in dir_list:
        if file[4:6] == 'VH' and int(file[2:3])==num:
            ...
            s1, s2 = ndvi0.shape
            print(colored(ndvi0.shape, "yellow"))
            """
            print(type(s1))
            print(type(ps))
            print(type(r))
            ...
            print(colored(f"x_train_k.shape: {x_train_k.shape}", "yellow"))
            print(colored(f"x_train.dim: {x_train.ndim}", "yellow"))
            print(colored(f"x_train_k.dim: {x_train_k.ndim}", "yellow"))
