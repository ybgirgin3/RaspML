...
def vidmain(src):
    ...
    while True:
        ...
        if cv2.waitKey(1) == ord('q'):
            #img1 = NDVI(frame, output_file=output_file or False, colors=colors or False)
            img = convertVid(frame)
            plt.imshow(img)
            plt.title('CAMERA')
            plt.xticks([])
            plt.yticks([])
            plt.show()
            mpimg.imsave('image.jpg', img)
            break                    
...
if __name__ == "__main__":
    """
    çalıştırma:
    resim için: python3 NDVI_CAM_PIC.py R -i input_file.jpg -o out_file.jpg
    kamera için
    """
    if sys.argv[1] == 'V':
        vidmain(sys.argv[2])
